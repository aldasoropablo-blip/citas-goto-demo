...
const BRAND_NAME = "Fisioterapia DPM";
const BRAND_SHORT = "Fisioterapia DPM";
const citas = [];
...
const PROPERTY_DETAILS = [
  { id: "1", nombre: "Fisioterapia Polanco", ubicacion: "Campos Eliseos, Polanco, Miguel Hidalgo, CDMX", tipo: "Fisioterapia ortopedica", precio: "Valoracion, terapia manual y rehabilitacion fisica", horario: "13:00" },
  { id: "2", nombre: "Fisioterapia Satelite", nombreVisible: "Fisioterapia Satélite", ubicacion: "Ciudad Satelite, Naucalpan, Estado de Mexico", ubicacionVisible: "Ciudad Satélite, Naucalpan, Estado de México", tipo: "Rehabilitacion deportiva", precio: "Evaluacion funcional, terapia fisica y ejercicios de recuperacion", horario: "15:00" },
  { id: "3", nombre: "Fisioterapia Santa Fe", ubicacion: "Avenida Santa Fe, Cuajimalpa, CDMX", tipo: "Rehabilitacion fisica integral", precio: "Terapia fisica, prevencion de lesiones y seguimiento personalizado", horario: "17:00" },
  { id: "4", nombre: "Fisioterapia Interlomas", ubicacion: "Jesus del Monte, Interlomas, Huixquilucan, Estado de Mexico", ubicacionVisible: "Jesús del Monte, Interlomas, Huixquilucan, Estado de México", tipo: "Fisioterapia musculoesqueletica", precio: "Valoracion, rehabilitacion y terapia especializada", horario: null }
];
...
app.post("/api/citas", async (req, res) => {
  ...
  if (!nombre || !telefono || !correo || !tipo || !fecha || !hora || !propiedad) return res.status(400).json({ error: "nombre, telefono, correo, sucursal, tipo, fecha y hora son obligatorios" });
  try {
...
app.post("/api/calendar/book", async (req, res) => {
  ...
  if (!nombre || !telefono || !correo || !propiedad || !fecha || !hora) return res.status(400).json({ error: "nombre, telefono, correo, sucursal, fecha y hora son obligatorios" });
  try {
    const cita = await createConfirmedAppointment({ ...req.body, propiedad, tipo: clean(req.body.tipo) || "Valoracion inicial", comentarios: appendPropertyToComments(req.body.comentarios, propiedad), origen: "Calendario en vivo" });
    res.status(201).json(toCreateResponse(cita));
...
app.post("/api/ivr/solicitudes", async (req, res) => {
  ...
  res.json({ ok: true, action: "solicitud_contacto", message: `Hemos registrado tu solicitud. El equipo de atencion de ${BRAND_NAME} se pondra en contacto contigo a la brevedad.` });
});
...
function resolvePropertyFromPayload(payload) { const direct = normalizePropertyName(payload.propiedad); if (direct) return direct; const match = clean(payload.comentarios).match(/(?:Propiedad|Sucursal)(?: seleccionada)?:\s*([^\n]+)/i); return match ? normalizePropertyName(match[1]) : "Fisioterapia Polanco"; }
...
function normalizePropertyInterest(value) { const map = { 1: "Fisioterapia Polanco", 2: "Fisioterapia Satélite", 3: "Fisioterapia Santa Fe", 4: "Fisioterapia Interlomas" }; const propiedad = clean(value); return normalizePropertyName(map[propiedad] || propiedad || "No especificada"); }
...
function appendPropertyToComments(comentarios, propiedad) { const base = clean(comentarios); const line = `Sucursal: ${propiedad}`; return base.includes(line) ? base : base ? `${line}\n${base}` : line; }
...
function buildWhatsAppUrl(cita) { const phone = cita.telefono.replace(/[^\d]/g, ""); const message = [`Hola, ${cita.nombre}. Tu cita con ${BRAND_NAME} fue registrada correctamente.`, "", `Folio: ${cita.folio}`, cita.propiedad ? `Sucursal: ${cita.propiedad}` : "", cita.ubicacion ? `Ubicacion: ${cita.ubicacion}` : "", `Tipo de cita: ${cita.tipo}`, `Fecha: ${cita.fecha}`, `Hora: ${cita.hora}`, "", "Para cambios o seguimiento, conserva tu folio y contacta al equipo de atencion."].filter(Boolean).join("\n"); return `https://wa.me/${phone}?text=${encodeURIComponent(message)}`; }
...
function customerEmailText(cita) { return [`Nombre: ${cita.nombre}`, `Folio: ${cita.folio}`, `Sucursal: ${cita.propiedad || "Por confirmar"}`, cita.ubicacion ? `Ubicacion: ${cita.ubicacion}` : "", `Tipo de cita: ${cita.tipo}`, `Fecha: ${cita.fecha}`, `Hora: ${cita.hora}`, "", "Conserva tu folio para cualquier cambio o seguimiento de tu cita.", "", `Gracias por contactar a ${BRAND_NAME}.`].filter(Boolean).join("\n"); }
function internalEmailText(cita) { return [`Folio: ${cita.folio}`, `Nombre del paciente: ${cita.nombre}`, `Telefono: ${cita.telefono}`, `Correo: ${cita.correo}`, `Tipo de cita: ${cita.tipo}`, `Sucursal: ${cita.propiedad || "Por confirmar"}`, cita.ubicacion ? `Ubicacion: ${cita.ubicacion}` : "", `Fecha: ${cita.fecha}`, `Hora: ${cita.hora}`, `Comentarios: ${cita.comentarios || "Sin comentarios"}`, `Estatus: ${cita.estatus}`, `Fecha de creacion: ${cita.createdAt}`, cita.calendarEventId ? `Google Calendar event: ${cita.calendarEventId}` : "", `emailSent: ${Boolean(cita.emailSent)}`, `internalEmailSent: ${Boolean(cita.internalEmailSent)}`].filter(Boolean).join("\n"); }
function cancellationCustomerEmailText(cita) { return [`Nombre: ${cita.nombre}`, `Folio: ${cita.folio}`, `Sucursal: ${cita.propiedad || "Por confirmar"}`, cita.ubicacion ? `Ubicacion: ${cita.ubicacion}` : "", `Fecha: ${cita.fecha}`, `Hora: ${cita.hora}`, "Estatus: Cancelada", "", "Tu cita ha sido cancelada correctamente."].filter(Boolean).join("\n"); }
function cancellationInternalEmailText(cita, calendarResult) { return [`Folio: ${cita.folio}`, `Nombre del paciente: ${cita.nombre}`, `Telefono: ${cita.telefono}`, `Correo: ${cita.correo}`, `Sucursal: ${cita.propiedad || "Por confirmar"}`, cita.ubicacion ? `Ubicacion: ${cita.ubicacion}` : "", `Fecha: ${cita.fecha}`, `Hora: ${cita.hora}`, `Comentarios: ${cita.comentarios || "Sin comentarios"}`, `Origen: ${cita.origen || "web"}`, "Estatus: Cancelada", `Calendar event id: ${cita.calendarEventId || "No disponible"}`, `CalendarReleased: ${Boolean(calendarResult.calendarReleased)}`].filter(Boolean).join("\n"); }
function serveBrandedHtml(res, filename) { fs.readFile(path.join(__dirname, "public", filename), "utf8", (error, html) => { if (error) return res.status(500).send("No se pudo cargar la pagina."); res.type("html").send(applyCurrentBrand(html)); }); }
function applyCurrentBrand(html) { return html; }
